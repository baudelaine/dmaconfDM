 ...
 */
@WebServlet(name = "ZipWKS", urlPatterns = { "/ZipWKS" })
public class ZipWKSServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

    /**
     * @see HttpServlet#HttpServlet()
     */
    public ZipWKSServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Path dlDir = Paths.get(getServletContext().getRealPath("/datas"));
			
			Path zip = Paths.get(dlDir + "/workspace-" + user + ".zip");
			...
			if(Files.exists(zip)) {
				...
				result.put("MESSAGE", "Workspace saved successfully in " + zip.getFileName());
